...
// extern const hsv_color_t hsv_palette_aurora[HSV_PALETTE_SIZE]; // Example for future
...
uint8_t palette_index_multiply(uint8_t contrast, uint8_t gentle, uint8_t modifier); // (contrast * gentle) / 255
uint8_t palette_index_average(uint8_t contrast, uint8_t gentle, uint8_t modifier); // (contrast + gentle) / 2
uint8_t palette_index_blend(uint8_t contrast, uint8_t gentle, uint8_t modifier);   // (1-z)contrast + z*gentle, z=modifier/255
